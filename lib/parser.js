...
function extractPrice(html, text) {
  ...
  // Priorité 1: Chercher TOUS les prix en EUR d'abord (formats variés)
  if (text) {
    // Regex plus flexible pour capturer tous les formats: "393.000€", "393,000€", "393 000€", "393000€"
    const eurRegex = /([\d\s,\.]+)\s*€/gi;
    let match;
    while ((match = eurRegex.exec(text)) !== null) {
      ...
      // Validation: prix doit avoir au moins 3 chiffres et être > 1000€
      if (priceNum > 1000 && cleanPrice.length >= 3) {
        eurPrices.push({
          original: match[0],
          clean: priceNum,
          display: priceStr + '€'
        });
        console.log('🔍 Prix EUR trouvé dans texte:', match[0], '→', priceNum);
      }
    }
  }
  ...
  if (html) {
    const eurRegex = /([\d\s,\.]+)\s*€/gi;
    let match;
    while ((match = eurRegex.exec(html)) !== null) {
      const priceStr = match[1].trim();
      // Nettoyer et convertir en nombre
      const cleanPrice = priceStr.replace(/[\s,\.]/g, '');
      const priceNum = parseInt(cleanPrice);
      ...
      if (priceNum > 1000 && cleanPrice.length >= 3) {
        ...
        console.log('🔍 Prix EUR trouvé dans HTML:', match[0], '→', priceNum);
      }
    }
  }
  
  // Si on a des prix en EUR, retourner le plus bas (prix de départ)
  if (eurPrices.length > 0) {
    // Trier par prix croissant et prendre le plus bas
    eurPrices.sort((a, b) => a.clean - b.clean);
    const lowestPrice = eurPrices[0];
    console.log('🔍 Prix EUR trouvés:', eurPrices.map(p => `${p.display} (${p.clean})`));
    console.log('🔍 Prix retenu (le plus bas):', lowestPrice.display, '=', lowestPrice.clean);
    console.log('🔍 Tous les prix originaux trouvés dans l\'email:');
    eurPrices.forEach((p, i) => {
      console.log(`  ${i + 1}. ${p.original} → ${p.clean}`);
    });
    return lowestPrice.display;
  }
