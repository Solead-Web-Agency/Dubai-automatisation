...
function extractSurface(html, text) {
  // Priorité 1: Chercher "Surface:" dans le texte
  if (text) {
    const surfaceMatch = text.match(/Surface:\s*([\d,]+)\s*(m²|sqm)/i);
    if (surfaceMatch) {
      return surfaceMatch[1].trim();
    }
  }

  // Priorité 2: Chercher "Surface:" dans le HTML
  if (html) {
    const surfaceMatch = html.match(/Surface:\s*([\d,]+)\s*(m²|sqm)/i);
    if (surfaceMatch) {
      return surfaceMatch[1].trim();
    }
  }
