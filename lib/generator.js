...
function safeReadAsset(filename) {
  ...
  try {
    // 1) dossier assets/
    const p1 = resolveAssetPath('assets', filename);
    console.log('  - Test path 1 (assets):', p1);
    if (fs.existsSync(p1)) return fs.readFileSync(p1);
    // 2) racine du projet
    const p2 = resolveAssetPath(filename);
    console.log('  - Test path 2 (racine):', p2);
    if (fs.existsSync(p2)) return fs.readFileSync(p2);
    // 3) sous-dossier /public
    const p3 = resolveAssetPath('public', filename);
    console.log('  - Test path 3 (public):', p3);
    if (fs.existsSync(p3)) return fs.readFileSync(p3);
    // 4) dossier courant de ce module
    const p4 = path.resolve(__dirname, '..', filename);
    console.log('  - Test path 4 (module):', p4);
    if (fs.existsSync(p4)) return fs.readFileSync(p4);
  } catch (e) {
    console.warn('safeReadAsset error', filename, e.message);
  }
  console.log('❌ Asset introuvable:', filename);
  
  // Fallback: créer une image par défaut si l'asset n'est pas trouvé
  console.log('🔄 Création image par défaut pour:', filename);
  return createDefaultAsset(filename);
}

function createDefaultAsset(filename) {
  const canvas = createCanvas(200, 200);
  const ctx = canvas.getContext('2d');
  
  // Fond coloré selon le type d'asset
  if (filename.includes('qrcode')) {
  ...
  } else if (filename.includes('bloctexte')) {
    ctx.fillStyle = '#00247d';
    ctx.fillRect(0, 0, 200, 200);
    ctx.fillStyle = '#ffffff';
    ctx.font = '16px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('BLOG TEXTE', 100, 100);
  } else {
...
async function ensureMontserratFont() {
  ...
  try {
    ...
    if (fs.existsSync(localPath)) {
      ...
      console.log('✅ Police locale chargée');
      return;
    }
  } catch (_) {}
  ...
  try {
    console.log('  - Téléchargement police...');
    ...
    if (fs.existsSync(tmpPath)) {
      ...
      montserratAvailable = true;
      console.log('✅ Police téléchargée et enregistrée');
    }
  } catch (e) {
...
async function generateStoryAd({ title, price, location, type, propertyImageBuffer, filename }) {
  ...
  try {
    ...
    ctx.fillRect(70, 36, 188, 6.8);
      ...
      console.log('✅ Texte "À DUBAI DÈS 250.000€" dessiné');
