...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename }) {
  ...
  try {
    ...
    const whiteSectionHeight = 428; // Hauteur de la partie blanche (1080 - 652)
    ctx.fillStyle = 'white';
    ctx.fillRect(0, 0, width, whiteSectionHeight);
    console.log('✅ Fond blanc créé: 1080x428px en haut');

    // Charger et dessiner l'image de la propriété (format carré 1080x652 fixée en bas)
    try {
      ...
      // Format carré : image de 1080x652 fixée en bas
      const imageHeight = 652;
      const imageY = height - imageHeight; // Fixée en bas (1080 - 652 = 428px du haut)
      ctx.drawImage(propertyImage, 0, imageY, width, imageHeight);
      console.log('✅ Image propriété dessinée en bas: 1080x652 à Y=', imageY);
    } catch (error) {
    ...
    try {
      ...
      // Position Y au milieu entre le fond blanc et l'image
      const blocY = (whiteSectionHeight - blocHeight) / 2; // Centré dans la partie blanche
