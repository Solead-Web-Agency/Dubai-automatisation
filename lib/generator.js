...
async function generateAds(propertyData, options = {}) {
  ...
  const { squareText, storyText, format, cartouche } = options || {};
  ...
  try {
    ...
    if (format === 'square' || !format) {
      results.square = await generateSquareAd({ title, price, location, type, propertyImageBuffer, filename: `${safeTitle}-square-${timestamp}.png`, squareText, cartouche });
      console.log('✅ Format carré généré');
    }
    
    if (format === 'story') {
      results.story = await generateStoryAd({ title, price, location, type, surface, propertyImageBuffer, filename: `${safeTitle}-story-${timestamp}.png`, storyText, cartouche });
...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename, squareText }) {
  ...
  try {
    ...
    // Créer le bloc cartouche dynamique au lieu de l'image statique
    if (cartouche && cartouche.trim()) {
      try {
        console.log('🔍 Création bloc cartouche dynamique...');
        const cartoucheText = cartouche.trim().toUpperCase();
        
        // Mesurer la largeur du texte pour calculer la largeur du bloc
        applyFont(ctx, { sizePx: 24, weight: 'bold' });
        const textMetrics = ctx.measureText(cartoucheText);
        const textWidth = textMetrics.width;
        
        // Dimensions du bloc : hauteur fixe 90px, largeur dynamique
        const blocHeight = 90;
        const blocWidth = Math.max(575, textWidth + 40); // Largeur minimale 575px, sinon texte + padding
        const blocX = 91.5; // Position X fixe
        const blocY = 280; // Position Y fixe
        
        // Dessiner le fond du bloc cartouche
        ctx.fillStyle = '#c89116';
        ctx.fillRect(blocX, blocY, blocWidth, blocHeight);
        
        // Dessiner le texte en blanc
        ctx.fillStyle = 'white';
        ctx.textAlign = 'center';
        const textX = blocX + (blocWidth / 2);
        const textY = blocY + (blocHeight / 2) + 8; // Centrer verticalement + petit décalage
        ctx.fillText(cartoucheText, textX, textY);
        
        console.log('✅ Bloc cartouche dynamique dessiné:', { text: cartoucheText, width: blocWidth, height: blocHeight });
      } catch (error) {
        console.warn('Impossible de créer le bloc cartouche:', error.message);
      }
    } else {
      console.log('ℹ️ Aucun cartouche fourni, bloc non affiché');
    }
...
async function generateStoryAd({ title, price, location, type, surface, propertyImageBuffer, filename, storyText }) {
  ...
  try {
    ...
    if (cartouche && cartouche.trim()) {
      try {
        console.log('🔍 Création bloc cartouche dynamique (story)...');
        const cartoucheText = cartouche.trim().toUpperCase();
        ...
        console.log('✅ Bloc cartouche dynamique (story) dessiné:', { text: cartoucheText, width: blocWidth, height: blocHeight });
      } catch (error) {
        console.warn('Impossible de créer le bloc cartouche (story):', error.message);
      }
    } else {
      console.log('ℹ️ Aucun cartouche fourni, bloc non affiché (story)');
    }
