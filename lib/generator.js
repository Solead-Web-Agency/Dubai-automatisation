...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename }) {
  ...
  try {
    ...
    try {
      const qrCode = await loadImage(resolveAssetPath('qrcode_dubaiimmo.png'));
    ...
    try {
      const blocTexte = await loadImage(resolveAssetPath('bloctexte_dubaiimmo.png'));
