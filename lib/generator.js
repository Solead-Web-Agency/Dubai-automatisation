...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename }) {
  ...
  try {
    ...
    // Dessiner le texte (style et wording identiques au format story)
    try {
      // Lignes rouges déjà dessinées plus haut
      // Texte principal
      ...
      const xText = 70.1;
      const y1 = 70.1 + 80; // première ligne
      ctx.fillText('NOUVEAU PROJET', xText, y1);

      const y2 = y1 + 90; // deuxième ligne
