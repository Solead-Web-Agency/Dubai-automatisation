...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename }) {
  ...
  try {
    ...
    try {
      ...
      ctx.fillText('NOUVEAU PROJET', xText, y1);

      const y2 = y1 + 62;                // deuxième ligne
      const surfaceText = typeof surface !== 'undefined' && surface ? `APPARTEMENT ${surface}M2` : 'APPARTEMENT 70M2';
      ctx.fillText(surfaceText, xText, y2);

      ctx.fillStyle = '#ff2525';
      const y3 = y2 + 62;                // troisième ligne
      const priceText = price ? `À DUBAI DÈS ${price}` : 'À DUBAI DÈS 250.000€';
      ctx.fillText(priceText, xText, y3);

      console.log('✅ Texte (square) dessiné entre les lignes rouges avec décalage 12px: y1=', y1, ' y2=', y2, ' y3=', y3);
    } catch (error) {
