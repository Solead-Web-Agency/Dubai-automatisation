  ...
  try {
    ...
    try {
      ...
      if (line1) {
        ...
        ctx.fillText(line1, xText, currentY);
      }

      if (line2) {
        currentY = (line1 ? currentY + 62 : currentY); // si line1 existe, on avance l'interligne, sinon on reste
        ctx.fillStyle = '#00247d';
        ctx.fillText(line2, xText, currentY);
      }

      if (line3) {
        currentY = ((line1 || line2) ? currentY + 62 : currentY); // ajouter interligne si au moins une ligne déjà posée
        ctx.fillStyle = '#ff2525';
        ctx.fillText(line3, xText, currentY);
      }
