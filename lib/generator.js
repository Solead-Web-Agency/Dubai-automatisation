...
async function generateAds(propertyData, options = {}) {
  ...
  const { squareText, storyText } = options || {};
...
async function generateStoryAd({ title, price, location, type, surface, propertyImageBuffer, filename, storyText }) {
  ...
  try {
    ...
    // Texte: overrides story si fournis
    const xText = 70.1;
    applyFont(ctx, { sizePx: 80, weight: 'bold' });
    ...
    let y = 79 + 80;
    if (s1) { drawStyledText(ctx, xText, y, s1, '#00247d'); }
    if (s2) { y += 90; drawStyledText(ctx, xText, y, s2, '#00247d'); }
    if (s3) { y += 90; drawStyledText(ctx, xText, y, s3, '#ff2525'); }
