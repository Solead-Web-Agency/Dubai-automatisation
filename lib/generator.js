...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename }) {
  ...
  try {
    ...
    try {
      ...
      const line1 = (cfg && cfg.line1) ? cfg.line1 : 'NOUVEAU PROJET';
      ...
      const line2 = (cfg && cfg.line2) ? cfg.line2 : defaultSurface;
      ctx.fillText(line2, xText, y2);

      ctx.fillStyle = '#ff2525';
      const y3 = y2 + 62;                // troisième ligne
      const defaultPrice = price ? `À DUBAI DÈS ${price}` : 'À DUBAI DÈS 250.000€';
      const line3 = (cfg && cfg.line3) ? cfg.line3 : defaultPrice;
      ctx.fillText(line3, xText, y3);

      console.log('✅ Texte (square) dessiné avec config:', { line1, line2, line3 });
    } catch (error) {
