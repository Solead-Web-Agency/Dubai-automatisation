...
async function generateAds(propertyData, options = {}) {
  ...
  try {
    ...
    if (format === 'square' || !format) {
      results.square = await generateSquareAd({ title, price, location, type, propertyImageBuffer, filename: `${safeTitle}-square-${timestamp}.png`, squareText, cartouche: options.cartouche });
      console.log('✅ Format carré généré');
    }
    
    if (format === 'story') {
      results.story = await generateStoryAd({ title, price, location, type, surface, propertyImageBuffer, filename: `${safeTitle}-story-${timestamp}.png`, storyText, cartouche: options.cartouche });
...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename, squareText, cartouche }) {
  const width = 1080;
  const height = 1080;
  ...
  try {
    ...
    const cartoucheText = cartouche || 'IMMOBILIER À DUBAI';
...
async function generateStoryAd({ title, price, location, type, surface, propertyImageBuffer, filename, storyText, cartouche }) {
  const width = 1080;
