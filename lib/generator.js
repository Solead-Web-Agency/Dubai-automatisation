...
async function generateStoryAd({ title, price, location, type, propertyImageBuffer, filename }) {
  ...
  try {
    ...
    // Charger et dessiner l'image de la propriété (format portrait selon tes spécifications)
    try {
      const propertyImage = await loadImage(propertyImageBuffer);
      // Format portrait : image de 1080x1465 (pleine hauteur)
    ...
    try {
      const blocTexte = await loadImage('./bloctexte_dubaiimmo.png');
      // Format portrait : bloc texte de 1080x1400 décalé de 200px vers la droite
      const blocWidth = 1080;
      const blocHeight = 1400;
