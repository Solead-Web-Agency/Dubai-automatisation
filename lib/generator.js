...
async function generateSquareAd({ title, price, location, type, propertyImageBuffer, filename, squareText }) {
  ...
  try {
    ...
    try {
      ...
      // Lignes 1-2 en bleu, 3 en rouge
      ctx.fillStyle = '#00247d';
      const y1 = 21.9 + yOffset + fontSize;        // baseline première ligne
      const line1 = (squareText && squareText.line1) ? squareText.line1 : (cfg && cfg.line1 ? cfg.line1 : 'NOUVEAU PROJET');
      ctx.fillText(line1, xText, y1);

      const y2 = y1 + 62;                // deuxième ligne
      const defaultSurface = typeof surface !== 'undefined' && surface ? `APPARTEMENT ${surface}M2` : 'APPARTEMENT 70M2';
      const line2 = (squareText && squareText.line2) ? squareText.line2 : (cfg && cfg.line2 ? cfg.line2 : defaultSurface);
      ctx.fillText(line2, xText, y2);

      ctx.fillStyle = '#ff2525';
      const y3 = y2 + 62;                // troisième ligne
      const defaultPrice = price ? `À DUBAI DÈS ${price}` : 'À DUBAI DÈS 250.000€';
      const line3 = (squareText && squareText.line3) ? squareText.line3 : (cfg && cfg.line3 ? cfg.line3 : defaultPrice);
      ctx.fillText(line3, xText, y3);

      console.log('✅ Texte (square) dessiné avec config/overrides:', { line1, line2, line3 });
    } catch (error) {
